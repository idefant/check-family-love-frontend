...
const useLoading = (defaultStatus: TLoadingStatus = 'before') => {
  ...
  const reset = useCallback(() => setStatus(() => 'before'), []);

  const start = useCallback(() => setStatus(() => 'loading'), []);

  const stopOk = useCallback(() => setStatus(() => 'ok'), []);

  const stopFailed = useCallback(() => setStatus(() => 'failed'), []);
